...
public partial class MapInputProcessor : Node2D
{
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        {
            //GD.Print($"distance too small: {(eventMouseButton.Position - pressStartPosition).LengthSquared()} above threshold");
            return;
        }

        if (eventMouseButton.Index > 0)
        {
            //GD.Print("second touch point, discarding");
            return;
        }
